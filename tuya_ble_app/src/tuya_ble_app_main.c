...
static const char auth_key_test[] = "aOJ8Y97evNDmFFK3Mvg5ni3NHY9ix5I2";
static const char device_id_test[] = "tuyaff5381ebd3bc";
static const uint8_t mac_test[6] = {0x58,0xD0,0x70,0x4D,0x23,0xDC}; //The actual MAC address is : DC234D70D058
uint8_t rec_buff[255] = {0};
...
static void tuya_cb_handler(tuya_ble_cb_evt_param_t* event)
{
    ...
    case TUYA_BLE_CB_EVT_CONNECTE_STATUS:
        ...
        status_flag = event->connect_status;//user add
        break;
    case TUYA_BLE_CB_EVT_DP_WRITE:
        ...
        app_dp_handle(dp_data_array);//user add
...
void tuya_ble_app_init(void)
{
    ...
    ty_factory_flag = 1;//锟斤拷锟斤拷锟斤拷锟斤拷锟铰硷拷锟饺� 锟斤拷权锟斤拷
//    tuya_timer_start(TIMER_FIRST,1000);
    ...
    spoon_init();
    switch_setup_network();		// 断电配网
}

void app_exe()
{
	led_connect_status();	// 配网指示灯 闪烁未配网 常亮配网（指示灯颜色  黄色）
	get_battery_value();	// 电池电量检测 频率 10s/次

	user_defined_temp_contorl(spoon_state.temp);	// 自定义模式下对温度的控制，特定口味模式下不执行此函数
